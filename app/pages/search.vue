    ...
    <div class="container mx-auto px-4 py-8">
      ...
      <div class="mb-8">
        <form class="flex gap-4" @submit.prevent="performSearch">
          ...
          >
            <Icon v-if="pending" name="mdi:loading" class="w-5 h-5 animate-spin" />
      ...
      <div v-else-if="hasSearched && !hasResults" class="text-center py-16">
        ...
        <p class="text-gray-400 mb-8">
          No movies or TV shows found for "{{ currentSearchQuery }}". Try different keywords.
        </p>
      </div>
      ...
      <div v-else-if="searchResults">
        <div class="mb-8">
          ...
          <p class="text-gray-400">
            Found {{ searchResults.totalMovies }} movies and {{ searchResults.totalTVShows }} TV shows
          </p>
        </div>
        ...
        <div v-if="searchResults.movies.length > 0" class="mb-12">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            <AppMovie
              ...
              :image-url="movie.poster_path ? `https://image.tmdb.org/t/p/w220_and_h330_face/${movie.poster_path}` : '/moviePoster.jpg'"
        ...
        <div v-if="searchResults.tvShows.length > 0" class="mb-12">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
            <AppTVShow
              ...
              :image-url="show.poster_path ? `https://image.tmdb.org/t/p/w220_and_h330_face/${show.poster_path}` : '/moviePoster.jpg'"
              :rating="Math.ceil(show.vote_average * 10)"
            />
          </div>
          <div v-if="searchResults.tvShows.length > 12" class="text-center mt-6">
            <p class="text-gray-400">Showing first 12 results</p>
          </div>
        </div>
      </div>
      ...
      <div v-else class="text-center py-16">
        ...
        <p class="text-gray-400 mb-8">
          Search for your favorite movies and TV shows using the search box above.
        </p>
...
const { data: searchResults, pending, error, execute } = await useLazyFetch<SearchResults>('/api/search', {
...
const hasResults = computed(() => {
  return searchResults.value && (searchResults.value.movies.length > 0 || searchResults.value.tvShows.length > 0)
})
